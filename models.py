...
class PipelinedVAE(diffusers.AutoencoderKL):
    def parallelize(self, ipu_conf):
        ...
        self.quant_conv = poptorch.BeginBlock(
            self.quant_conv, "quant_conv", ipu_id=3
        )
...
class PipelinedUnet(diffusers.UNet2DConditionModel):
    def parallelize(self, ipu_conf):
        ...
        self.conv_in = poptorch.BeginBlock(
            self.conv_in, "conv_in", ipu_id=1
        )
        self.time_proj = poptorch.BeginBlock(
            self.time_proj, "time_proj", ipu_id=1
        )
        ...
        self.mid_block = poptorch.BeginBlock(
            self.mid_block, "mid_block", ipu_id=2
        )
        self.up_blocks = poptorch.BeginBlock(
            self.up_blocks, "up_blocks", ipu_id=2
        )
        
        self.conv_norm_out = poptorch.BeginBlock(
            self.conv_norm_out, "conv_norm_out", ipu_id=2
        )
        self.conv_act = poptorch.BeginBlock(
            self.conv_act, "up_blocks", ipu_id=2
        )
        self.conv_out = poptorch.BeginBlock(
            self.conv_out, "up_blocks", ipu_id=2
        )
